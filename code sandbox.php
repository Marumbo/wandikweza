...
function checkResponse ($userResponse, $menu_num, $con,$menu,$sessionId,$countOfArray,$textExplodeinit)
{
    if ($userResponse == "#" && $countOfArray > 1)
    {
        $menu_num -= 1;
        mysqli_query($con, "UPDATE $menu SET `menu_number` = $menu_num WHERE `session_id` = '$sessionId'");
        array_pop($textExplodeinit);
...
// function to add values to textExplode
// which is local app user repsonse tracker

function responseArray ($userResponse,$textExplode)
{

    if ($userResponse == "M" || $userResponse == "m")
    {
        array_push($textExplode,$userResponse);

    }
    else if($userResponse == "#")
    {
        array_pop($textExplode);
    }
    else if($userResponse != "")
    {
    ...
    else if ($userResponse == "" && count($textExplode) == 1)
    {
        $textExplode[0]= "";
    }
    else
...
$menu_number = checkResponse ($userResponse, $menu_num, $con,$menu,$sessionId,$countOfArray,$textExplodeinit);



$textExplode = responseArray($userResponse,$textExplode);
                ...
                if($textExplode[$countOfArray-3] ==1)
                {
                    ...
                    if (mysqli_query($con, $familyPlanning)) {
                        try {
                            ...
                            $response = "END Encountered an error while sending SMS: ".$e->getMessage();
